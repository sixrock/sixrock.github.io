...
1. 一个计算机平台包括一组硬件资源，比如说处理器、内存、I/O模块、定时器和磁盘驱动器等。
2. 计算机程序是为了执行某些任务而开发的。他们接受外来的输入，做一些处理之后，输出结果。
3. 直接根据给定的硬件平台写应用程序，所以开发出这些应用程序的访问组员的通用例程是有意义的。
4. 开发操作系统是为了给应用程序提供一个方便的、安全和一致的接口。操作系统是计算机硬件和应用程序之间的一层软件，对应用程序提供了支持。

有了应用程序是为了、系统软件和资源的概念，就可以谈论操作系统是怎样以一种有效的管理应用程序的执行，达到以下目的：
- 资源对多个应用是可用的。
...
也可以把进程看做由一组元素组成的实体，进程的两个基本元素是程序代码和代码相关的数据集。

假设处理器开始执行这个程序代码，且我们把这个执行体称为进程。在进程执行的过程中，任意给定时间，进程进程可以唯一地表征为以下元素：
- 标识符：跟这个进程相关的唯一标识符，用来区别其他进程。
- 状态：如果进程正在执行，那么进程处于运行状态。
- 优先级：相对于其他进程的优先级
- 程序计数器：程序中即将被执行的下一条指令的地址
- 内存指针：包括程序代码和进城相关的数据的指针，还有和其他进程共享的内存块的指针。
- 上下文数据：进程执行时处理器寄存器的数据。
- I/O状态信息：包括显式的I/O请求、分配给进程I/O设备和被使用的文件列表等。
前述进程列表信息被存放在一个称为进程控制块的数据结构中，该控制块由操作系统创建和管理。

进程状态

对一个被执行的程序，操作系统会为该进程创建一个进程或者任务。从处理器的角度来说，他在指令序列中按照某种顺序执行指令，这个顺序会根据程序计数器的不断变化的值来表示，程序计数器可能指向不同进程中的不同部分的程序代码。

两状态进程模型

操作系统的基本职责是控制进程的执行，包含确定交替执行的方法和给进程分配资源。在设计控制进程的程序时，第一步就是描述进程所表现出来的行为。

一个进程可以处在以下两种状态之一：运行态和未运行态。当操作系统创建一个新的进程的时候，他将该进程以运行态加入到系统中，操作系统知道这个进程是存在的，并正在等待执行机会。当前正在进行的进程被中断，操作系统中的分派器会将部分将选择一个新的进程执行。前一个进程从运行状态转换到未运行状态，另一个进程转换到运行态。

操作系统的设计元素，必须用某种方式来表示每个进程，使得操作系统能够跟踪它。包含进程在内存中的当前状态和位置，即进程控制块。

被中断的进程转移到等待进程队列中，或者，如果进程已经结束或者取消，则被销毁。在任何情况下，分派器均从队列中选择一条进程来执行。

进程的创建和终止

进程的创建

当一个新进程被添加到那些正在被管理的进程集合中去时，操作系统需要建立用于管理该进程的数据结构，并在内存中给它分配地址空间。
通常会有四种情况导致进程的创建
事件|说明
--:|:--
新的批处理作业|通常位于磁带或者磁盘中的批处理作业控制流被提供给操作系统。当操作系统准备接纳新的工作的时候，它将读取下一个作业的控制命令
交互登录|终端用户登录到系统
操作系统因为提供一项服务而创建|操作系统可以创建一个进程
由现有的进程派生|基于模块化的考虑，或者为了开发并行性，用户程序可以指示创建多个进程

操作系统创建进程是用户和应用程序都是透明的。允许一个进程引发另一个进程的创建是很有用的。一个应用程序可以产生另一个进程，可以接受应用程序产生的数据，并将程序组织成适于以后分析的格式，进程与应用程序并行的运行，并在得到新的数据的时候被激活。服务器进程可以为它处理的每一个请求产生一个新的进程。当操作系统为另一个进程的显式子请求创建一个进程的时候，这个动作叫做进程派生。

当一个进程派生另一个进程的时候，前一个进程叫做父进程，被派生的进程叫做子进程。

进程终止

任何一个计算机系统都必须为进程提供表示其完成的方法，批处理作业中包含了一个Halt指令或其他操作系统显式服务用来终止。对交互式应用程序，用户的行为将指出进程指出何时进程的完成。

导致进程终止的原因
事件|说明
--:|:--
正常完成|进程自行执行操作系统服务调用，表示它已经结束运行
超过时限|进程运行的时间超过规定的期限。
无可用内存|系统无法满足进程需要的内存空间
越界|进程试图访问不允许访问的内存单元
保护错误|进程试图不允许使用的资源或文件，或者以一种不正当的方式使用
算数错误|进程试图进行被禁止的计算
时间超出|进程等待某一事件的发生超过了规定的最大值
I/O失败|在输入输出期间发生错误
无效指令|进程试图执行一个不存在的指令
特权指令|进程试图使用操作系统保留的指令
数据误用|错误类型或未初始化的数据
父进程终结|当一个父进程终结的时候，操作系统可能会自动终止该进程的所有后代
父进程请求|父进程进程具有终止其任何后代的权利

五状态模型
- 运行态：该进程正在被执行，假设计算机只有一个处理器，因此最多有一个进程处于这个状态。
- 就绪态：进程做好了准备，只要有机会就开始执行
- 阻塞等待状态：进程在某些事件发生前不能执行，如I/O操作
- 新建状态：刚刚创立的进程，操作系统还没有把它加入到可以执行的进程组中。通常是进程控制块已经创建出但还没有加载到内存中的新进程。
- 退出态：操作系统从可执行进程组中解放出的进程，或者是因为自身停止了，或者因为某种原因被取消。

导致进程状态转换的事件类型：
- 空——新建：创建执行一个程序的新进程。
- 新建——就绪：操作系统准备好接纳一个新的进程，把一个进程从新建态转为就绪态。大多数系统基于现有的进程或分配给现有进程的虚存数量设置，以确保不会因为活跃的进程的数量过多而导致系统性能下降。
- 就绪——运行：需要选择一个新的进程运行的时候，操作系统选择一个处于就绪状态的进程，这是调度器或分派器的工作。
- 运行——退出：如果当前正在运行的进程表示完成或者取消，则它将被操作系统终止。
- 运行——就绪：这类转换最常见。
- 运行——阻塞：如果一个进程请求它必须等待某些事件，则进入阻塞状态。
- 阻塞——就绪状态：当所等待的事件发生时，处于阻塞状态的进程进入就绪状态。
- 就绪——退出：父进程可以随时终止一个子进程。如果一个父进程终止了，与该进程相关的所有子进程都被终止。

被挂起的进程

就绪态、运行态和阻塞态提供了一种为进程行为建立的系统方法，并指导操作系统的实现。

所有这集的原因都是I/O活动比计算机速度快的多，因此在单道程序系统的处理器大多是空闲的。

### 直接储存访问
DMA技术的工作流程：

当处理器想读或者写一块数据的时候，它通过向DMA模块发送以下信息来给DMA模块发出一条命令：

- 请求读操作或写操作的信号，通过在处理器和DMA模块之间使用读写控制线发送。
- 相关的I/O设备地址，通过数据线传送。
- 存储器中或者往存储器中写的起始地址，在数据线上传送时，并由DMA模块保存在其地址寄存器中。
- 读或写的字数，也通过数据线传送，并由DMA模块保存在其数据寄存器中。

然后处理器继续执行其他工作，此时他已经把这个操作委托给DMA模块。DMA模块直接从存储器中或往存储器中传送整块数据，一次传送一个字，并且数据不需要通过处理器。传送结束后，DMA给处理器发送一个中断信号。

DMA机制可以按照多种方法配置：

第一种，所有模块共享一个地址总线，DMA模块担当起处理器的作用，它使用程序控制I/O通过DMA模块 在存储器和I/O之间交换数据。尽管这个配置可能开销并不大但是效率很低：跟处理器控制的程序控制I/O一样，每传送一个需要两个周期。

第二种：通过把DMA和I/O模块功能集成起来，可以大大的减少所需要的总线周期数目。这就意味着除了总线系统以外，在DMA模块和一个或多个I/O模块之间还存在着一条不包含系统总线的路径。DMA逻辑实际上可能就是I/O模块的一部分，或者可能是I/O模块的一部分，过着可能是控制一个或多个I/O模块的一个单独模块。通过使用一个I/O总线连接I/O模块和DMA模块。

操作系统的设计问题

设计目标

在设计I/O机制时，有两个重要的目标：效率性和通用性

效率是重要的，这是因为I/O操作通常是计算机的瓶颈。与内存和处理器相比，大多数I/O设备速度是非常低的。解决这个问题的一种方法是多道程序设计。多道程序设计允许在一个执行的同时其他的一些进程在等待I/O操作。即使计算机拥有大量的内存，I/O操作跟不上处理器活动的情况。交换技术用于将额外的就绪进程加载到内存，从而保持处理器处于工作状态，但这本身就是一个I/O操作。I/O设计的主要目标就是提高I/O的效率

通用性出于简单和错误的避免，希望有一种统一的方式处理所有的设备。这意味着从两个方面都要统一，一个是处理器看待I/O设备的方式，另一个是操作系统管理I/O设备和I/O操作方式。由于设备特性的多样性，在实际中很难实现通用性。目前所能做到的就是用一种层次化、模块化的方法设计I/O功能。这种方法隐藏了大量的I/O设备底层例程中的细节。使得操作系统高层可以通过读、写、打开、关闭、加锁和解锁的等一些通过的函数来操作I/O设备。

I/O功能的逻辑结构 

首先考虑一种简单的情况，本地设备以一种简单的方式进行通信，如字节流或者记录流，会涉及到下面几层：

- 逻辑I/O：逻辑I/O模块把设备当做一个逻辑资源来处理，它并不关心实际设计的细节。
逻辑I/O模块代表用户进程管理的一般的I/O功能，允许用户根据设备标识符以及诸如打开、关闭、读、写之类的简单命令打交道。
- 设备I/O：请求的操作系统被转换成适当的I/O指令序列、通道命令和控制指令。可以使用缓冲技术来提高利用率。
- 调度和控制：I/O操作的队列、调度实际发生在这一层。因此在这一层处理中断，收集并报告I/O状态。这一层与I/O模块和设备硬件真正发生交互的软件层。
- 目录管理：在这一层，符号文件名被转换成标识符，通过标识符可以通过文件描述表或索引表直接或者间接的访问文件。这一层还影响文件目录的用户操作，如添加、删除和重新组织等。
- 文件系统：这一层处理文件的逻辑结构和用户指定的操作，如打开、关闭、读、写等。这一层还管理访问权限。
- 物理组织：考虑到辅存设备的物理磁道和扇区结构，对于文件记录的逻辑访问也必须转换成物理外存地址。

I/O缓冲

假设某个数据需要从磁盘中读入多个数据块，每次读入一块，每块的长度为512个字节。这些数据将被读入用户进程地址空间的某个区域，如果从虚拟地址1000到1511的区域。最简单的方法就是对磁盘单元执行一个I/O命令，并等待数据传输完毕。这个等待可以是忙等待，也可以是进程被中断挂起。

单缓冲

操作系统提供最简单的缓冲类型是单缓冲，当用户进程发出I/O操作的时，操作系统给该操作分配一个位于内存中系统部分的缓冲区。

单缓冲方案的描述：

输入传输的数据被放到系统缓冲区中，当传送完成时，进程把该块引动到用户空间，并立即请求另一块。这称为预读，或者预先输入。这样做的目的是这块数据终将被使用。对于许多计算机类型来说，这个假设在大多数情况下是合理的，因为数据通常是被顺序访问的，只有在处理序列的最后，才会读入一个不必要的块。

双缓冲

单缓冲的改进方案，可以给操作分配两个系统缓冲区，在一个进程往一个缓冲区中传输数据的同时，操作系统正在清空另一个缓冲区，这种技术叫做双缓冲，或者缓冲交换
对于面向块的传送，我们可以粗略的估计执行的时间在任何一种情况下，比单缓冲都有所提高，但是这种提高是以增加复杂性为代价的。

缓冲循环

双缓冲方案可以平滑I/O设备和进程之间的数据流。如果关注的焦点是某个特定的进程的性能，那么常常会希望相关I/O操作能够跟得上这个进程。如果该进程需要爆发性地执行大量的I/O操作，仅有双缓冲是不够的，这种情况下会用多于两个的缓冲区来解决不足。

当使用两个以上的缓冲区的时候，这组缓冲区被当做循环缓冲区，其中的每一个缓冲区就是这个循环缓冲区的一个单元。

缓冲的作用：
缓冲是用来平滑I/O需求的峰值的一种技术，但是当一个进程的平均需求大于I/O设备的服务的时候，缓冲再多也不能让I/O设备和这个进程并驾齐驱。即使有多个缓冲区，所有的缓冲区终将会被填满进程在处理完一大块数据的时候不得不等待。再多道进程环境中，当存在多种I/O活动和多进程时，缓冲是提高操作系统和单个进程性能的一种方法。

磁盘调度

磁盘性能的参数

磁盘I/O的实际操作细节取决于计算机系统、操作系统以及I/O通道和磁盘控制器硬件的特性。

当磁盘驱动器工作的时候，磁盘以一种恒定的速度旋转为了读或写，磁头必须位于指定的磁道和该磁道中指定的扇区的开始处。磁道选择包括在活动头系统中移动磁头或者在固定头系统中电子选择一个磁头。在活动头系统中，磁头定位到磁道的时间称为寻道时间。

在任何一种情况下，一旦选择好磁道，磁盘控制器就开始等待，直到适当的扇区旋转到磁头的时候，磁头到达扇区开始位置的时候称为旋转延迟。寻道时间和旋转延迟的总和为存取时间，这是到达读或者写需要的时间。一旦磁头完成，磁头就通过下面旋转的扇区，开始执行读操作或者写操作。传输所需的时间是传输时间

除了存取时间和传输时间之外，一次磁盘I/O操作通常还会有排队延迟。当一个进程发出一个I/O请求的时候，他必须首先在一个队列中等待该设备的可用。再适合的时候，该设备分配给这个进程。如果该设备与其他磁盘驱动器共享一个I/O通道或一组I/O通道，还可能需要额外的等待时间，直到该通道可用。在这之后才开始访问磁盘。
